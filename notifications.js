	...
	follow.getFollowList(req, res, function(data2) {
		...
		follow.getProjectFollowedBy(req, res, function(result) {
          ...
          project.getMyInvolvedProject(req, res, function(data6) {
  			    var allList = data.concat(data2, data3, data4, data5, data6);
...
exports.getNotifications = function(req, res) {
	...
	if (result.done) {
	    pool.query("SELECT * FROM notification_list WHERE user_id = ? ORDER BY date_of_view DESC", [req.user.id], function(err, data) {
		if (err) throw err;
		var notifArray = [];
		function recursive(index) {
		    if (data[index]) {
