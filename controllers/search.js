            ...
            if (data[index]) {
                function recursive2(index2) {
                    if (data[index][index2]) {
                        pool.query('SELECT id, first_name, last_name, description, location_city, location_state, location_country, profile_picture, about, cover_picture_cards FROM profiles WHERE id IN (SELECT profile_id FROM users WHERE id = ?)',
...
exports.getUsersByAl = function(req, res) {
    if (req.body.about || req.body.geo) {
        pool.query('SELECT id, first_name, last_name, description, location_city, location_state, location_country, profile_picture, about, cover_picture_cards FROM `profiles` WHERE fake = 0 ORDER BY views DESC', 
            function (err, results) {
            if (err) throw (err);
            if (results[0]) {
