  ...
  (function () {
      ...
      function Users($http, $cookieStore, $rootScope, $resource, $q) {
          ...
          function getLastMessage() {
            ...
            return $http.get('http://127.0.0.1/messages/get/all')
          ...
          function count() { // count number of unread dialogue
            $http.get('http://127.0.0.1/messages/get/all').success(function(res) {
              if (res.success) {
          ...
          function getUserIdByProfileId(profile_id) {
            ...
            return $http.get('http://127.0.0.1/userId/' + profile_id)
              .then(function(response) {
                defer.resolve(response.data);
          ...
          function getUsers() {
              ...
              data = $resource('http://127.0.0.1/users');
          ...
          function getProfiles() {
              ...
              data = $resource('http://127.0.0.1/profiles');
              data.query().$promise.then(function(data) {
                  ret = data;
                  defer.resolve(ret);
              });
          ...
          function getCardProfiles(callback) {
            $http.get('http://127.0.0.1/user/card/profiles').success(function(res) {
              callback(res);
            });
          };
          function getProfileView(username) {
            $http.put('http://127.0.0.1/profiles/view/' + username);
          };

          function getProfileByUserId(user_id, callback) {
            $http.post('http://127.0.0.1/profileId/' + user_id).success(function(response) {
              callback(response);
          ...
          function getProfilesByProfileId(profile_id, callback) {
            $http.post('http://127.0.0.1/profiles/' + profile_id).success(function(response) {
              callback(response);
            }).error(function(error_message) {
              callback(error_message);
            });
          ...
          function getUserbyId(user_id, callback) {
            $http.get('http://127.0.0.1/user/'+ user_id).success(function (response) {
              callback(response);
            });
          };
          ...
          function getUserbyIdUnresolved(user_id) {
              if (user_id) {
                  return $http.get('http://127.0.0.1/user/' + user_id);
              } else {
                  return null;
              }
          }
      		...
      		function updateUser(user_id, data, callback) {
                ...
                $http.put('http://127.0.0.1/user/'+ user_id, data).success(function (response) {
                  callback(response);
